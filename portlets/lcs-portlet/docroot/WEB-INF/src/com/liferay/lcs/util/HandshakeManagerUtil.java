/**
 * Copyright (c) 2000-2012 Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package com.liferay.lcs.util;

import com.liferay.portal.kernel.exception.PortalException;

import java.util.concurrent.Future;

/**
 * @author Ivica Cardic
 */
public class HandshakeManagerUtil {
	public static boolean isPending() {
		return _handshakeManager.isPending();
	}

	public static boolean isReady() {
		return _handshakeManager.isReady();
	}

	public static Future<?> start() throws PortalException {
		return _handshakeManager.start();
	}

	public static Future<?> stop() {
		return _handshakeManager.stop();
	}

	public void setHandshakeManager(HandshakeManager handshakeManager) {
		_handshakeManager = handshakeManager;
	}

	private static HandshakeManager _handshakeManager;

}